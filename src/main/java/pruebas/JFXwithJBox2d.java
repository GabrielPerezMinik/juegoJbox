...
public class JFXwithJBox2d extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        final Scene scene = new Scene(root, TheWorld.WIDTH, TheWorld.HEIGHT,Color.BLACK);
        
        //Ball array for hold the  balls
        final Square[] square = new Square[TheWorld.NO_OF_SQUARE];
         ...
         */
        for(int i=0;i<TheWorld.NO_OF_SQUARE;i++) {
        	square[i]=new Square(r.nextInt(90)+5,r.nextInt(400)+100);
        }
     
        //Add ground to the application, this is where balls will land
        TheWorld.addGround(100, 10);
        
        //Add left and right walls so balls will not move outside the viewing area.
        TheWorld.addWall(0,100,1,100); //Left wall
        TheWorld.addWall(99,100,1,100); //Right wall
        ...
        EventHandler<ActionEvent> ae = new EventHandler<ActionEvent>() {
            public void handle(ActionEvent t) {
            	...
            	TheWorld.world.step(1.0f/60.f, 8, 3); 
                       
                       //Move balls to the new position computed by JBox2D
                       for(int i=0;i<TheWorld.NO_OF_SQUARE;i++) {
                            Body body = (Body)square[i].node.getUserData();
                            float xpos = TheWorld.toPixelPosX(body.getPosition().x);
                            float ypos = TheWorld.toPixelPosY(body.getPosition().y);
                            square[i].node.setLayoutX(xpos);
                            square[i].node.setLayoutY(ypos);
                       }
           }
        };
        ...
        btn.setLayoutX((TheWorld.WIDTH/2));
        btn.setLayoutY((TheWorld.HEIGHT-30));
        btn.setText("Start");
        ...
        for(int i=0;i<TheWorld.NO_OF_SQUARE;i++) {
            root.getChildren().add(square[i].node);
        }
        ...
        EventHandler<MouseEvent> addHurdle = new EventHandler<MouseEvent>(){
            public void handle(MouseEvent me) {
                    ...
                    Square hurdle = new Square(TheWorld.toPosX(dragX), TheWorld.toPosY(dragY),2);
